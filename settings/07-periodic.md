...
| P14 | Dialogue carriage & relationship pressure | Optional | 인물 둘 이상 장면에서 대사와 반응만으로도 상황, 위계, 숨은 의도, 관계 마찰이 일정 부분 읽히는지 확인. 장르적으로 대화 비중이 낮은 작품이면 Optional로 둔다 |
| P15 | Pressure-release rhythm | Optional | 최근 화수의 톤이 계속 눌리기만 하지 않는지 점검. 작품이 의도적으로 침잠형 리듬을 택하지 않았다면, 짧은 완충 호흡이나 관계성 리듬의 부재를 drift 후보로 본다 |
| P16 | Co-presence / off-screen logic | Optional | 같은 방, 숙소, 식사, 동행, 경계, 은신처를 공유하는 이름 있는 인물이 장면에서 상식선으로 처리됐는지 확인. 있어야 할 인물이 투명인간처럼 사라지지 않았는지 점검 |
...
5. dialogue carriage & relationship pressure (if relevant)
6. pressure-release rhythm (if relevant)
7. co-presence / off-screen logic (if relevant)
8. repetition
9. Korean naturalness
10. external review reconciliation (if used)
11. summary re-sync
