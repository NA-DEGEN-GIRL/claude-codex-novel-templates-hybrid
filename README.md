...
| MCP 도구 | 직접 사용 | **scripts/ CLI wrapper** |
...
> **Codex도 MCP를 네이티브로 지원한다.** `codex mcp add`로 서버를 등록하면 Claude Code와 동일하게 MCP 도구를 사용한다. `scripts/`는 MCP 미설정 시 fallback. Writer가 MCP를 못 쓰더라도 Claude review 세션이 동일 검증을 수행한다 (이중 안전망).
