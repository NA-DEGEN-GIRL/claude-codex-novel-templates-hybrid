...
- `summaries/desire-state.md`, `summaries/signature-moves.md`는 선택적 live field다. 켜 두면 `compile_brief`의 `Live Drafting Cues`가 더 구체적이 된다.
...
- `settings/`: 소설 자체의 규칙 정본
- `CLAUDE.md`: 프로젝트 약속/금지/운영 기본값 정본
- `batch-supervisor.md`: hybrid 런타임 순서, sentinel, review gate 정본
- `README.md`: 빠른 이해와 온보딩용. 절차 충돌 시 위 3개 문서를 우선한다.
...
│                          │  │  (:: run={RUN_NONCE})     │  │  REVIEW_DONE sentinel      │
└──────────┬───────────────┘  └─────────────▲─────────────┘  └──────────▲───────────────┘
           │                                │                           │
           ├── 집필 프롬프트 ──────────────→ │                           │
           ├── fix-spec 수정 지시 ─────────→ │                           │
...
│                              ├─ 축약 자기점검
│                              ├─ chapter 파일 저장
│                              └─ WRITER_DONE :: run=... ─→ (supervisor 감지)
│                                                              │
├──── post-write 지시 ──────────────────────────────────────→ │
│                                                              ├─ unified-reviewer
│                                                              ├─ external AI review
│                                                              │   (Gemini + NIM + GPT* + Proxy*)
│                                                              ├─ 문제 발견 시:
│                                                              │   fix-spec 생성
│ ←─── fix routing 요청 ──────────────────────────────────────┤
│                                                              │
...
```
                       ...
                       └───────────────┬───────────────┘
                                       │
                       ┌───────────────▼───────────────┐
                       │  Review 세션에 post-write 지시 │
                       │  unified-reviewer (review_floor│
                       │  에 맞는 모드로 실행)          │
                       └───────────────┬───────────────┘
                                       │
                     ┌─────────────────▼─────────────────┐
                     │  Specialist trigger 판정           │
                     │                                    │
                     │  plot risk tag 읽기 (plot/{arc}.md) │
                     │  + continuity ESCALATE_* flag 읽기 │
                     │                                    │
                     │  tag+flag → 즉시 실행              │
                     │  flag만  → 실행 (emergent)         │
                     │  tag만   → watch (보류)            │
                    ...
                    └──────────┬──┘       │
                     ...
                     ┌─────────────▼───────────────────┐
                     │  후처리 (review 세션)             │
                     │  summary 갱신 + EPISODE_META     │
                     │  + dialogue-log + git commit     │
                     │  → REVIEW_DONE :: run=...        │
                     └─────────────┬───────────────────┘
                                   │
                     ┌─────────────▼───────────────────┐
...
| 완료 감지 | `❯` 프롬프트 | **`WRITER_DONE ... :: run={RUN_NONCE}` sentinel** |
| summary/META/commit | writer가 수행 | **supervisor/review 세션이 수행** |
| 외부 리뷰 | Claude + Gemini + GPT + NIM | **writer_model에 따라 조합 조정** (+ Proxy 선택) |
...
4. [PLOT-REINFORCEMENT-PROMPT.md](./PLOT-REINFORCEMENT-PROMPT.md)로 `plot/`을 집필 직전 수준으로 강화
5. 필요하면 [FULL-STORY-PROMPT.md](./FULL-STORY-PROMPT.md)로 `full_story.md`를 생성
6. 생성된 소설 폴더의 `batch-supervisor.md`를 기준으로 supervisor 실행
7. Claude supervisor가 `writer_model`에 맞는 writer 세션을 만들고 집필 시작
...
- writer가 끝나면 `WRITER_DONE chapter-XX.md :: run={RUN_NONCE}` 형태의 exact sentinel을 출력한다.
...
| MCP | 역할 | 사용 시점 |
|-----|------|----------|
| [mcp-novel-editor](https://github.com/NA-DEGEN-GIRL/mcp-novel-editor) | 외부 AI 리뷰 (Gemini/GPT/NIM/Proxy line-edit), compile_brief | 매 화 리뷰 + 아크 통독 |
