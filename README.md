...
- [.claude/prompts/codex-writer-role.md](./.claude/prompts/codex-writer-role.md): Codex 창작 역할 + 문체 원칙
- [.claude/prompts/codex-writer.md](./.claude/prompts/codex-writer.md): Codex writer 집필 프롬프트 (정본)
- [.claude/prompts/codex-fixer.md](./.claude/prompts/codex-fixer.md): Codex fixer 수정 프롬프트
...
# 예: 런타임에서는 MCP 직접 호출이 기본이다.
# shell wrapper는 수동 디버깅이 필요할 때만 보조적으로 사용한다.
```
...
> **Codex도 MCP를 네이티브로 지원한다.** `codex mcp add`로 서버를 등록하면 Claude Code와 동일하게 MCP 도구를 직접 사용한다. `scripts/`는 수동 셸 테스트가 필요할 때만 보조적으로 둔다. Writer가 MCP를 누락해도 Claude review 세션이 동일 검증을 수행한다.
