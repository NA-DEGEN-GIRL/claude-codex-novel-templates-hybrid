...
- GPT가 실제로 집필할 모델이므로, "이 컨셉을 내가 쓴다면 어떤 점이 좋고 어떤 점이 어렵겠다"는 실행 관점 피드백이 특히 유용
- GPT 의견을 참고하여 시나리오 보강 또는 새 시나리오 추가 가능
...
**에피소드 집필은 하지 않는다. 파일 생성 후 3.7단계 Writer 정밀 검토로 넘어간다.**

### 3.7단계: Writer(GPT) 정밀 검토

`3단계`의 5번(`settings/` 작성) 완료 직후, **Codex writer(GPT 5.4)**가 settings 핵심 3종을 검토한다 (`ask_gpt` MCP 활용). 이 단계에서는 본문을 쓰지 않는다.
...
> **왜 이 단계가 필요한가**: Voice Profile 하나의 차이가 소설 전체의 문체를 바꾼다. Claude가 만든 settings를 실제 writer인 GPT가 검토하지 않으면, "규칙은 맞지만 안 읽히는" 문체가 고착된다. 초기 세팅에서 잡는 비용이 50화 뒤에 고치는 비용보다 압도적으로 낮다.
...
- CLAUDE.md의 분량 목표와 `.claude/prompts/codex-writer.md`의 분량이 일치하는가
...
4. 내가 OK하면 claude-codex-novel-templates-hybrid/의 구조대로 전체 파일을 생성해줘.
   ...
   - config.json 등록
5. 파일 생성 후, **Writer(GPT) 정밀 검토** (`ask_gpt` MCP):
...
프롬프트 1의 3~5단계 절차 전체를 수행하고, 3.7단계 Writer(GPT) 정밀 검토 + /oag-check plan + /why-check plan + 교차 검증도 수행해.
모순 발견 시 즉시 수정.
...
## 복제 대상
- 원본: no-title-[번호]
- 복제할 것: CLAUDE.md, settings/, .claude/agents/ (unified-reviewer.md, full-audit.md, narrative-reviewer.md), .claude/prompts/ (codex-writer.md, codex-fixer.md), .claude/commands/ 폴더, plot/master-outline.md, plot/foreshadowing.md
...
- batch-supervisor.md 생성 (WRITER_CMD: [집필 모델 명령])
  예시: "codex --dangerously-bypass-approvals-and-sandbox" (hybrid 기본), "claude" (lean fallback)
...
프로젝트 생성이 완료되면, **Claude supervisor + Codex writer** 구조로 집필을 시작한다:
...
Claude supervisor가 tmux 세션에서 Codex(GPT 5.4)를 실행하고 자동 관리한다.
`WRITER_CMD`는 `codex --dangerously-bypass-approvals-and-sandbox` (hybrid 기본값).
