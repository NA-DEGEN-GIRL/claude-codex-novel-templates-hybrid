...
[읽기 — 반드시]
0. .claude/prompts/claude-writer-role.md — 역할과 공통 문체 원칙
1. CLAUDE.md — 금지사항, 시대감, 핵심 약속 (§3.3 review와 §3.4 후처리는 읽되 수행하지 않는다)
...
5. 직전 화 마지막 장면 — 최소 2~3문단. auto-compact 여부와 무관하게 마지막 장면 전체 또는 마지막 8~12문단을 우선 확인해 오프닝 연결
...
[집필 전 메모 — 짧게]
- `compile_brief`의 `Live Drafting Cues`를 먼저 훑는다.
- 이번 화의 기능 1줄
- 직전 화에서 반드시 이어받을 사실 3~5개
- 이번 화 시작 시점의 공개 정보 / 비공개 정보 2~4개
- 이미 처리된 일 / 아직 처리되지 않은 일 2~4개
...
- 전투/위기 장면에서는 감상적 압축보다 행동의 선명도를 우선한다.
- 인물 둘 이상 장면에서는 대사가 상황, 관계, 숨은 의도 중 최소 하나를 직접 운반해야 한다.
- 기본 캐릭터 톤보다 화자-청자 위계와 관계 register를 먼저 맞춘다. 부모/연장자/사부/상급자 앞에서는 관계가 읽혀야 한다.
- 같은 공간의 이름 있는 인물을 투명인간처럼 지우지 않는다.
- 직전 화 마지막 장면에서 아직 전달되지 않은 정보나 완료되지 않은 조치를 오프스크린에서 끝난 일처럼 쓰지 않는다.
- 보고/허락/안심/소문 확산/관아 전달/관계 변화는 본문에 보인 경우에만 다음 화의 기정사실로 사용한다.
...
- 부모/연장자/동년배/아랫사람/적대자 앞에서 대사가 한 톤으로 평탄화되지 않았는가?
- 호칭, 존대/반말, 짧은 대답이 관계 위계와 충돌하지 않는가?
- 조연이 아직 들은 적 없는 사실을 알고 행동하지 않는가?
- 보고/허락/안심/소문 확산/관아 전달이 오프스크린에서 이미 끝난 것처럼 점프하지 않았는가?
...
- plot/{arc}.md와 직전 화 마지막 장면을 다시 확인. 마지막 장면 전체 또는 마지막 8~12문단 우선.
- 파일명: chapters/{arc}/chapter-{NN}.md
- `Live Drafting Cues`를 먼저 읽고, 이번 화 기능/직전 화 carry-forward 사실/공개·비공개 정보/보이스 압력/주요 장면/엔딩 훅을 짧게 정리하고 집필.
- Drafting Surface를 먼저 따르고, Hard Rules는 위반 금지선으로만 유지한다.
- 잘 쓰려 하기보다 잘 읽히게 쓴다. 첫 문장, 전투 문장, 장면 전환 문장을 특히 평이하고 선명하게 유지한다.
- `settings/03-characters.md`와 `CLAUDE.md §8.1` 기준으로 관계별 register를 먼저 맞춘다.
- 오프스크린 완료 처리 금지: 보고/허락/안심/소문 확산/관아 전달/관계 변화는 본문에 보였을 때만 다음 화 기정사실로 사용.
- 초안 후 분량 확인 + 기능 수행/반복 설명/결합 자연성/전투 선명도/공간 공유 인물 + carry-forward 점프만 짧게 점검.
- run nonce: `{RUN_NONCE}`
- 완료 문자열 접두: `WRITER_DONE chapter-{NN}.md`
- chapter 저장 후 `mkdir -p tmp/sentinels && printf '%s\n' "WRITER_DONE chapter-{NN}.md :: run={RUN_NONCE}" > tmp/sentinels/chapter-{NN}.done` 실행
- 마지막 줄 exact 형식은 `<접두> :: run=<run nonce>` 이다.
