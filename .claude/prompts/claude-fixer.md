# Claude Fixer Prompt Template

> **🏷️ 모드 라벨 (Phase 3 role consolidation, 2026-04-17)**: 이 파일은 `writer_model: claude` 전용. `codex`에는 `codex-fixer.md`.
>
> Claude supervisor가 진단한 문제를 Claude writer가 문체를 유지하며 수정한다.
> Claude supervisor는 fix-spec 파일을 생성하고, Claude writer는 그 파일을 읽고 수정만 수행한다.

---

> **이 파일에는 `patch_class: micro`, `local`, `rewrite`가 모두 온다.**
> 문체 일관성을 위해 사실관계 micro-patch 포함 모든 텍스트 수정은 writer가 수행한다.
> `hold`만 제외 (다음 사이클 이관).
...
> Claude supervisor가 생성. Claude writer는 이 파일만 읽고 수정.
...
- 대표 문단 앵커: {§0.3 문단 A의 첫 1~2문장 — patch_class에 rewrite가 하나라도 있으면 해당 문단 전체를 첨부}
- 이 화의 POV: {인물명}
- 수정 범위 밖 문장 변경 금지
- 여러 항목이 있으면 line_start가 큰 항목부터 처리
- style/naturalness 성격 항목의 "수정 목표"는 방향 서술이 기본이다 — 리뷰어의 완성 대체문이 적혀 있어도 골격만 참고하고 자기 보이스로 재작성한다
```
