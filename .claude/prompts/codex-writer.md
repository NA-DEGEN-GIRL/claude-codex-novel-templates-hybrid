...
- 단, **한국어 결합 자연성은 보이스보다 우선**한다. 무게를 만들려는 압축 때문에 주어-서술어, 명사-동사, 추상명사-행위 결합이 어색해지면 평이한 문장으로 되돌린다. "보이스 같아 보인다"는 이유로 이상한 결합을 살려두지 않는다.
...
- 문장 하나하나를 천천히 읽었을 때, **뜻은 통하지만 한국어 화자가 실제로는 거의 쓰지 않는 결합**이 없는가? 첫 문단, 장면 전환 첫 문장, 문단 마지막 문장에서 특히 확인한다.
- 첫 문장이 분위기를 의식한 나머지 물리적 대상과 성질의 결합을 비틀고 있지 않은가? 평이하게 풀어 써도 장면 힘이 유지되면 그쪽을 택한다.
...
- 초안 후 자기점검: 연속성(즉흥 설정, POV 경계, 불변 조건, 현재 시점/경과일수/회상 나이/이동·회복 기간, 같은 공간의 이름 있는 인물 처리) + 서사 기능(장면 기능 수행, WHY/HOW, 첫 문단 견인력, hook 중복, 반복 설명) + 한국어 1회 훑기(번역투, 약한 동사, 어미 반복, 서수/외래어, **결합 자연성**).
